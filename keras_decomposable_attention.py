...
from keras.layers import Bidirectional, GRU, LSTM, Masking
...
def build_model(x1_shape, input_dims, embedding_matrix):
    '''Compile the model.'''
    # max_length, nr_hidden, nr_class = shape
    # Declare inputs.
    ids1 = Input(shape=x1_shape[1:], dtype="int32")
    ids2 = Input(shape=x1_shape[1:], dtype="int32")

    max_length=40
    
    # Construct operations, which we'll chain together.
    embed = _StaticEmbedding(embedding_matrix, input_dims)
    attend = _Attention(max_length, 100, dropout=0.2)
    align = _SoftAlignment(max_length, 100)
    compare = _Comparison(max_length, 100, dropout=0.2)
    entail = _Entailment(100, dropout=0.2)
...
class _StaticEmbedding(object):
    def __init__(self, embedding_matrix, input_dims):
...
class _Comparison(object):
    ...
    def __call__(self, sent, align, **kwargs):
        ...
        # result = Masking(input_shape=(self.words,))(result)
...
class _GlobalAveragePooling1D(Layer):
    '''Global sum pooling operation for temporal data.
    ...
    '''
    def __init__(self, **kwargs):
        super(_GlobalAveragePooling1D, self).__init__(**kwargs)
        self.input_spec = [InputSpec(ndim=3)]
    ...
    def call(self, x, mask=None):
        if mask is not None:
            return K.mean(x * K.clip(mask, 0, 1), axis=1)
        else:
            return K.mean(x, axis=1)


class _GlobalMaxPooling1D(Layer):
    '''Global sum pooling operation for temporal data.
    ...
    def call(self, x, mask=None):
        if mask is not None:
            return K.max(x * K.clip(mask, 0, 1), axis=1)
        else:
